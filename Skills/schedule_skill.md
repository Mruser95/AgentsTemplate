...
- **执行者固定为 manager**：`Tools/Tasker_schedule.py` 永远 `from Agents.manager import manager_agent` 来执行到点任务。其他 agent（coder / checker / …）**不允许**被 schedule 直接唤醒。
...
- 包装脚本：`.schedule/<task_id>.bat`（Win）或 `.sh`（Linux）—— 只做 `cd` + `python -m Tools.Tasker_schedule --task <id>`。用包装脚本是为了躲开 `schtasks /TR` 的嵌套引号规则。
- 运行日志：`.schedule/<task_id>/<yyyymmdd_HHMMSS>.log` —— 运行器执行完 / 挂掉都会写。
- 运行器：`Tools/Tasker_schedule.py` —— **固定** `importlib.import_module("Agents.manager")` 后取 `manager_agent`，把 `intent + context` 拼成一条 `HumanMessage` 塞给它。不再根据 creator 字段动态 import 任何 agent。
