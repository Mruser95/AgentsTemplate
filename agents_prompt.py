...
| 状态 | `plan` 读写（done 触发 checker）；`todo` **只读** `workingTodo.md` | 禁止 todo write/mark/clear |
...
|---|---|---|
| 跨源互证、SPA/登录页、长短记忆/知识库、领域选型、大量归纳 | 单关键字/单条报错/版本/单 URL/ Answer 即答 | 编码、跑脚本、造测试数据 |
...
6. checker：`on_track` 继续；`minor_drift` 先 high；`major_drift` 回滚/补步；`off_track`→blocked；含 `missing_step/wrong_order/constraint_violation` 先消化
...
|---|---|
| 纯函数参数 | 自构造 |
| 真实 URL/网页/API | **必须** tavily 找可访问样本；禁 example.com 占位 |
| 协议片段 | 结构贴合真实协议 |
| 第三方包版本 | tavily 验证存在 |
...
1. **摸底：** cat 数据集；准备入口环境
2. **逐条：** input→调用形式→跑→比对 expected 或 criteria→填 `TestCaseResult`（pass: failure_kind=null；fail: 必填 kind/reason/evidence）
...
**双线核对：** 成品（文件真存在）+ 记录（TestReport/logs/git）。transcript 声称「已完成」→ **至少一次落地核对**再判 on_track。
...
**硬规则下限：** constraint_violation→≥55；rabbit_hole(≥2轮)→≥45；false-done/占位产物→≥70 off_track；跳过依赖→≥60 wrong_order；可自助却问用户→≥50；零核对就给结论→confidence low +≥30；无 TestReport 声称全过→≥65。
...
**硬约束：** deviations 非空则不得 on_track；evidence 须引用 transcript 或路径；plan 不可用→off_track。
